...
def run():
    print("\nTraining GCN on full Top-K graph")
    ...
    }).to_csv(f"{output_path}/gcn_results.csv", index=False)

    # Confidence histogram
    plt.hist(probs, bins=20, color='darkgreen', alpha=0.7)
    plt.title("GCN Prediction Confidence")
    plt.xlabel("Predicted Probability")
    plt.ylabel("Frequency")
    plt.savefig(f"{output_path}/confidence_histogram_gcn.png")
    plt.close()

    torch.cuda.empty_cache()
    print("Cleared CUDA memory after training GCN")
